    ...
    <aside className="bg-dk2 border-r border-dk4 overflow-y-auto h-screen sticky top-0 w-[400px] shrink-0">
      ...
      <SidebarSection icon="👤" title="Buyer Profile" collapsible={false}>
        ...
        <Field label="Monthly Household Income ($)">
          <SliderBox value={`$${Number(inc).toLocaleString()} / mo`} rawValue={inc} min={0} max={21000} step={500} onChange={setNum('inc')} left="$0" right="$21,000" />
        </Field>
      ...
      <SidebarSection icon="💰" title="Budget" collapsible={false}>
        ...
        <Field label="Max Monthly Loan Repayment ($)">
          <SliderBox value={`$${Number(loan).toLocaleString()} / mo`} rawValue={loan} min={500} max={6000} step={100} onChange={setNum('loan')} left="$500" right="$6,000" />
        </Field>
        {loanLimitWarning && (
